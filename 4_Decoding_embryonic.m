...
analysisSavePath = SetFigureSavePath([dataPath,'\FOV\'], ...
     'makeDir', true);
RawPath = ''; % Rawdata File Path
DAPIPath=''; % dapi or nuclear images path for segmentation
% Useful data structure for spotfinding
all=readtable(''); % readout probe file including the color and image round for each readout
...
bitnumber=24;
roundnumber=8; %Three color, 8 rounds of imaging
znumber=30; % Z stack number
...
if exist([dataPath 'illumination.mat'],'file')
    ...
    Path=[RawPath];
    file=['Epi-750s30-650s30-555s30-488s30_(?<fov>[0-9]+)_0'];    %% example file name
  ...
  %  
    Path=DAPIPath;
    file=['Epi-750s1-650s1-560s1-488s1-405s1_(?<fov>[0-9]+)_0'];     %% example file name
    tempFiles = BuildFileStructure(Path, ...
    'fileExt', 'dax', ...
    'regExp', file, ...
    'fieldNames', {'fov','round'}, ...
    ...
    if ~exist(strcat(mapPath,['FOV_' num2str(fovID),'data.mat']),'file')
       ...
       warning('off','all')
        ...
        for bit=1:bitnumber
            ...
            switch color(bit)
                case 750
                    a=ReadDax([RawPath,'Epi-750s30-650s30-555s30-488s30_',num2str(fovID-1,'%03d'),'_', num2str(desiredround(bit),'%01d'),'.dax'], 'startFrame', 1, 'endFrame', znumber,'verbose',false);                  
                    a=double(a)./ratio750/100;
                case 650
                    a=ReadDax([RawPath,'Epi-750s30-650s30-555s30-488s30_',num2str(fovID-1,'%03d'),'_', num2str(desiredround(bit),'%01d'),'.dax'], 'startFrame', znumber+1, 'endFrame', znumber*2,'verbose',false);  
                    a=double(a)./ratio647/100;
                    a=flip(a,3);
                case 561
                    a=ReadDax([RawPath,'Epi-750s30-650s30-555s30-488s30_',num2str(fovID-1,'%03d'),'_', num2str(desiredround(bit),'%01d'),'.dax'], 'startFrame', znumber*2+1, 'endFrame', znumber*3,'verbose',false);  
           ...
           oldbw=bw4;
           thre=0.99;
           for threshold=0.99995:-0.0001:thre
...
%% find the spot and barcode 
      ...
      spotlocalization={};
       ...
       spoton=zeros(10000,bitnumber);
               ...
               spotbit=[];
                for bit=1:bitnumber
                    allspot=[allspot;[bitinfo(bit).x',bitinfo(bit).y',bitinfo(bit).z'.*2]];
...
plotPath=SetFigureSavePath([analysisSavePath,'\individual_loci\'], ...
     'makeDir', true);
[~,~,temp]=xlsread(''); % Codebook path
region=temp(2:end,8);
cat=temp(2:end,9);
badfov=[141,142,143,140,289,134,78,73,74]; %% those fovs are out of the brain tissue
for b=1:150
    figure;
...
[sortednA, sind] = sort(count, 'descend');
    ...
    ylabel('Counts');
%     xlim([0 141]);    
    maxBlank = max(count(147:end));
    title(['Number above: ' num2str(sum(count(1:147) > maxBlank))]);
